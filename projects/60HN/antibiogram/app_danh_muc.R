...
# Tìm dòng tiêu đề trong 10 dòng đầu: dòng đầu tiên có giá trị ở cột B (index 2)
detect_header_row <- function(path) {
  tryCatch({
    probe <- openxlsx::read.xlsx(path, colNames = FALSE, rows = 1:10)
    hit <- which(!is.na(probe[, 2]) & trimws(as.character(probe[, 2])) != "NA")
