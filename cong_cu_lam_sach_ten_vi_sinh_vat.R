...
REF_FILE <- "G:/Group Folders/ACADEMIC/Controlled documents/60HN - ASPARNet/7. DATA/ANTIBIOGRAM/DTH/1.danh_muc/DTH_danh_muc_vsv.xlsx"
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      h4("1) Nguồn dữ liệu"),
      p("Dữ liệu gốc: nhiều file WHONET (cột Organism)"),
      p("Danh mục chuẩn hóa: Danh mục Vi sinh vật"),
...
server <- function(input, output, session) {
  ...
  raw_df <- reactive({
    ...
    all_org <- lapply(files, function(f) {
      ...
      # Chuẩn hóa tên cột (tránh lỗi khoảng trắng)
    ...
    validate(
      need(length(all_org) > 0, 
           "Không file nào có cột 'Organism'")
    )
    
    out <- bind_rows(all_org)
    
    validate(
      need("ma_hoa" %in% names(out), 
           "Không tạo được cột ma_hoa")
    )
